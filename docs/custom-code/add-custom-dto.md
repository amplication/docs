...
DTOs can be defined by TypeScript interfaces or by classes. Thought, it is preferable to use classes as TypeScript interfaces are removed during the transpilation, and can't be referred a at runtime. However, classes are part of the JavaScript ES6 standard, and therefore they are preserved as real entities in the compiled JavaScript.

The DTOs in Amplication are generated based on the relations between the entities and Prisma schema and types.
Sometimes you have use-cases where you need DTOs that are supported by Prisma but not generated by Amplication yet.
...
## Adding a new DTO
1. Determind which DTO you need. For example, let's say you have an application with Customer entity and you want to filter all the customers with the `customerType` property - "Gold". On the generated app you will get a DTO named `CustomerFindManyArgs` with a field (for GraphQL) or ApiProperty (for REST) from type: `CustomerWhereInput`
...
`EnumCustomerCustomerType` is only an enum, and not a DTO, meaning you can't filter with it. 
...
2. Find the right DTO - according to Prisms `EntityWhereInput` wraps all model fields in a type so that the list can be **filtered by any property**.
...
3. Create a custom DTO from this type:
    ...
    - inside this folder, create a 2 new files: `CustomerWhereInputWithFilterEnum.dto.ts` and `EnumEnumCustomerCustomerTypeFilter.dto.ts`
    - in `EnumEnumCustomerCustomerTypeFilter.dto.ts` file create a new class `EnumEnumCustomerCustomerTypeFilter` with the follwing properties: (equals, in, notIn, not) as you can see above.
    ...
    - this class should looks exactly like `CustomerWhereInput`, except from the type of `customerType` field/API property. In this case, its type should be `EnumEnumCustomerCustomerTypeFilter`
...
```
As mentioned earlies, this class should looks exactly like `CustomerWhereInput`. Therefore, you need to copy and paste the other properties from the `CustomerWhereInput` DTO.
